...
- **Single operator.** Codex runs the whole pipeline end to end. There is no
...
- **State lives in files, never terminal memory.** The durable record is: the
  data artifacts, `.Codex/ops/jobs/` job cards, and the dashboard. Re-derive
...
- **Current production dataset (Neon `archi_data`):**
  ...
  - Cleanup history: job card
    `.Codex/ops/jobs/20260524_neondb_cleanup_role_separation.md`.
...
- **Record every non-trivial run** as a job card in `.Codex/ops/jobs/`
...
- `.Codex/ops/jobs/` — job cards = run history
