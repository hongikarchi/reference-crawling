#!/usr/bin/env python
"""Validate one terminal E2 cross-source image-evidence SQLite artifact."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Independently validate a terminal E2 evidence artifact and its "
            "recorded input files without network access or database writes."
        )
    )
    parser.add_argument("artifact", type=Path, help="terminal E2 SQLite artifact")
    parser.add_argument(
...
def _payload(report: EvidenceValidationReport) -> dict[str, Any]:
...
def _json_safe(value: Any) -> Any:
    """Recursively convert report values to deterministic JSON-safe values."""
...
def _print_human(report: EvidenceValidationReport) -> None:
    print("E2 VALIDATION: " + ("PASS" if report.passed else "FAIL"))
    print(f"artifact: {report.artifact_path}")
    print(f"run_id: {report.run_id or '-'}")
    print(f"logical_sha256: {report.logical_sha256 or '-'}")
    print(f"checks: {sum(check.passed for check in report.checks)}/{len(report.checks)} passed")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print(
                f"E2 VALIDATION ERROR: {type(exc).__name__}: {exc}",
                file=sys.stderr,
            )
        return 2
    if args.json:
