...
DEFAULT_OUTPUT = CCR / "canonical_buildings_strict_embedded.completeness_c24_search_quality.json"
DEFAULT_KEYWORDS = REPORT_DIR / "canonical_v2_search_keywords.c24.jsonl"
DEFAULT_MATERIAL_REVIEW = REPORT_DIR / "material_unmapped_review.c24.jsonl"
DEFAULT_REPORT = REPORT_DIR / "search_quality_c24_report.json"
DEFAULT_MD = REPORT_DIR / "search_quality_c24_report.md"
DEFAULT_MAPPING = REPORT_DIR / "material_mapping.c24.json"
...
def material_mapping_payload(raw_material_counter: Counter[str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": [
            ...
            "Unknown material terms are removed from material_visual and reported for later LLM/manual review.",
        ],
    }
...
def apply_cleanup(
    *,
                    ...
                    json.dumps(
                        {
                            ...
                            "review_reason": "no controlled material_visual remains after deterministic cleanup",
                        },
                        ensure_ascii=False,
                    )
                    + "\n"
    ...
    report = {
        ...
        "validation_commands": [
            f"python3 tools/canonical_v2_upload_validator.py --input {output_path} --report {REPORT_DIR / 'c24_upload_validator.json'}",
            f"python3 tools/canonical_v2_full_reaudit.py --input {output_path} --strict data/canonical/country_conflict_refresh/canonical_buildings_strict.completeness_c23_final.json --report {REPORT_DIR / 'c24_full_reaudit.json'} --md {REPORT_DIR / 'c24_full_reaudit.md'}",
            f"python3 tools/canonical_v2_neon_loader.py --dry-run-upsert --input {output_path} --report {REPORT_DIR / 'c24_neon_dry_run_upsert.json'}",
        ],
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    lines = [
        ...
        f"- material unmapped review rows: {report['material_unmapped_review_rows']}",
        f"- material unmapped publishable review rows: {report['material_unmapped_publishable_review_rows']}",
        "",
        "## Search Keywords",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply deterministic C24 search-quality cleanup")
