    ...
    with get_db() as conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS divisare_projects (
                ...
                slug            TEXT UNIQUE NOT NULL,
                name            TEXT NOT NULL,
                architect_ids   TEXT,                       -- JSON array of architect ids
                architect_names TEXT,                       -- JSON array of names (display)
                location_country TEXT,
                location_city    TEXT,
                project_year     INTEGER,
                published_date   TEXT,                      -- ISO from "Published on …"
                abstract         TEXT,
                description      TEXT,
                album_name       TEXT,                      -- e.g. "Atlas of Architecture"
                tag_slugs        TEXT,                      -- JSON array
...
def upsert_project(data: dict) -> None:
    """Insert or update a row in divisare_projects. JSON-serializable list fields
    (architect_ids, architect_names, tag_slugs) get json.dumps applied."""
    json_fields = ("architect_ids", "architect_names", "tag_slugs")
    ...
    cols = ["id", "slug", "name", "architect_ids", "architect_names",
            ...
            "published_date", "abstract", "description", "album_name",
            "tag_slugs", "cover_image_url"]
